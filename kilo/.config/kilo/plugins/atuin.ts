/**
 * Atuin plugin for Kilo (vendored from Atuin's opencode plugin, v18.20.1:
 * crates/atuin/contrib/opencode/atuin.ts).
 *
 * Tracks bash commands executed by Kilo in Atuin history with author
 * `opencode`.
 *
 * Local adaptations from upstream:
 *   - import from `@kilocode/plugin` (Kilo's fork of `@opencode-ai/plugin`)
 *   - install path is `~/.config/kilo/plugins/atuin.ts`
 *   - `--author-kind agent` omitted: atuin <= 18.19 rejects the flag and the
 *     spawn would silently fail; author name `opencode` still classifies the
 *     entry as agent-run. Restore it once atuin >= 18.20 is on this system.
 *
 * Regenerate from upstream with `atuin hook install opencode` (needs atuin
 * >= 18.20) and re-apply these changes. See ai-docs/atuin-kilo-hook.md.
 */
...
// Kilo's shell tool keeps the id `bash` for compatibility, even though its
...
// A rejected hook aborts Kilo's tool call and discards the command's
...
// Kilo treats every export of a plugin file as a plugin function and fails
// to load the file if one is not, so keep this the only export.
export const AtuinPlugin: Plugin = async ({ directory }) => {
	// Commands Kilo has proposed but is not yet cleared to run, keyed by
	// tool call ID.
	...
	return {
		...
		// Kilo did not run; those are skipped because they have no
